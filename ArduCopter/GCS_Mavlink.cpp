...
bool GCS_MAVLINK_Copter::try_send_message(enum ap_message id)
{
	...
	case MSG_NAV_CONTROLLER_OUTPUT:
		...
		break;
		//Zomato code change
	case MSG_PA_INTERNAL_ID:
		CHECK_PAYLOAD_SIZE(INTERNAL_ID);
		copter.send_internal_id(chan);
		break;
	case MSG_PA_LOG_GF_BR:
		CHECK_PAYLOAD_SIZE(PA_GEOFENCE_BREACH_LOG);
		copter.send_log_geofence_breach(chan);
		break;
	case MSG_PA_LOG_TAKEOFF:
		CHECK_PAYLOAD_SIZE(PA_TAKEOFF_LOG);
		copter.send_log_takeoff(chan);
		break;
	case MSG_PA_LOG_LAND:
		CHECK_PAYLOAD_SIZE(PA_LAND_LOG);
		copter.send_log_land(chan);
		break;
	case MSG_RFM_STATUS:
		CHECK_PAYLOAD_SIZE(RFM_STATUS);
		copter.send_time_breach(chan);
		break;
...
void GCS_MAVLINK_Copter::handleMessage(mavlink_message_t* msg)
{
	...
	case MAVLINK_MSG_ID_PA_GEOFENCE:
		copter.rfm.handle_pa_geofence_points(*this,msg,copter.fence);
		break;
	...
	//this is custom handling of messages from sbc.
	case MAVLINK_MSG_ID_MESSAGE_ACK:
	{
		mavlink_message_ack_t packet;
	...
	case MAVLINK_MSG_ID_SBC_HEARTBEAT:
	{
		mavlink_sbc_heartbeat_t packet;
		mavlink_msg_sbc_heartbeat_decode(msg,&packet);
		copter.rfm.sbc_hb.PA_received_status=packet.PA_received_status;
		...
		if((copter.rfm.sbc_hb.PA_received_status && copter.rfm.sbc_hb.PA_verification_status && copter.rfm.sbc_hb.PA_sent_to_fc_status)||
				(copter.rfm.sbc_hb.PA_received_status && copter.rfm.sbc_hb.PA_verification_status))
		{
			copter.rfm.sbc_ready_to_send_PA=true;
			mavlink_msg_sbc_heartbeat_send(chan,copter.rfm.sbc_hb.PA_received_status,copter.rfm.sbc_hb.PA_verification_status,copter.rfm.sbc_hb.PA_sent_to_fc_status);
		}
		else
			copter.rfm.sbc_ready_to_send_PA=false;

		break;
	}

	case MAVLINK_MSG_ID_DATE_TIME_BOUND:
	{
		if(!copter.rfm.sbc_ready_to_send_PA)
		{
			return;
		}
		mavlink_date_time_bound_t packet;
		mavlink_msg_date_time_bound_decode(msg,&packet);

		/* sanity check!
		 * PA_end_time or PA_start_time should never be 0
		 */
