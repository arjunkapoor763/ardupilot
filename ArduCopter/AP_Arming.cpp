...
bool AP_Arming_Copter::arm_checks(bool display_failure, AP_Arming::ArmingMethod method)
{
	...
	//an additional NPNT chneck, Zomato code change
	if(checks_to_perform == ARMING_CHECK_ALL || checks_to_perform == ARMING_CHECK_NPNT)
	{
		/*********************************SBC_ALIVE CHECK*****************************/
		if(!copter.rfm.sbc_alive)
		{
			if(display_failure)
			{
				check_failed(ARMING_CHECK_NPNT,display_failure,"Permission Artefact not Available");
			}
			return false;
		}
		if(!copter.res_time_veri)
		{
			if(display_failure)
			{
