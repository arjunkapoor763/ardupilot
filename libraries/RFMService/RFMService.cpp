...
void RFMService::handle_pa_geofence_points(mavlink_channel_t chan, mavlink_message_t *msg, AC_Fence &fence)
{
	...
	//    		if(once == false && _lcl_count<=total_vertices)
	//        	{
	//    			_ret_lat = _ret_lat + packet.Latitude;
	//    			_ret_lng = _ret_lng + packet.Longitude;
	//
	//        			if(_lcl_count++ == total_vertices)
	//        			{
	//        				_ret_lat=(_ret_lat / total_vertices);
	//        				_ret_lng=(_ret_lng / total_vertices);
	//        				_return_point.x=_ret_lat*1.0e7f;
	//        				_return_point.y=_ret_lng*1.0e7f;
	//        				_lcl_count=1;
	//        				total_vertices=0;
	//        				_poly_loader.save_point_to_eeprom(0, _return_point);
	//        				_return_point.x=0;
	//        				_return_point.y=0;
	//        				_ret_lat=0;
	//        				_ret_lng=0;
	...
	//    		}
	//
	//    		_fence_count=packet.total_count+1; //total count needs to be filled from here
	//
	//    		//this is the ack/nak to every polygon point coming from sbc
	//    		result=MAV_RESULT_ACCEPTED;
	//    	}
	//    	//sending ack for received point from SBC
	//    	mavlink_msg_message_ack_send(chan,result,MAVLINK_MSG_ID_PA_GEOFENCE);
	//    	break;
	//    }
	//
	...
	//    		fence._total.set_and_save(_fence_count); //setting the total points to what is coming from sbce packet.
	//    		fence._enabled.set_and_save(1);
	//    		fence._enabled_fences.set_and_save(AC_FENCE_TYPE_CIRCLE_POLYGON);
	//    		fence._action.set_and_save(0);
	//    		mavlink_msg_fence_point_send_buf(msg, chan, msg->sysid, msg->compid, packet.idx, fence._total, point.x*1.0e-7f, point.y*1.0e-7f);
	//
	//    	}
	//    	else
	//    	{
